...
SDK and extension callers may configure `allowedRoots` on the built-in `read`, `edit`, `write`, `grep`, `find`, and `ls` tool factories. The tools canonicalize paths, reject symlink escapes, and revalidate mutation or search targets. This is an opt-in defense against accidental or confused-path file access; it is not a sandbox and cannot eliminate filesystem TOCTOU races against a hostile process. Use an OS, container, or VM boundary for adversarial workloads.
...
The boundary profile declares workspace mounts and read/write modes, process execution policy, network policy, and the environment-variable/secret names allowed to enter tool processes. The backend must attest that it enforces the exact profile digest and declare matching capabilities. Pi rejects profile mismatches, missing capabilities, missing tool operations, and attempts to replace boundary-owned operations. Canonical `allowedRoots` checks are still applied to bounded file and search tools as defense in depth. Bounded `grep` also requires a delegated backend search operation, so pi never starts host `rg` for a boundary-owned search. These checks are not treated as proof of isolation.
